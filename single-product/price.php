...
function ag_getProductVariations($siteURL, $product)
{
    $curl = curl_init();

    curl_setopt_array($curl, [
        CURLOPT_URL => "$siteURL/wp-json/wholesale/v1/products/$product->id/variations",
        CURLOPT_RETURNTRANSFER => true,
        CURLOPT_ENCODING => "",
        CURLOPT_MAXREDIRS => 10,
        CURLOPT_TIMEOUT => 30,
    ...
    if ($err) {
        ...
        $productDetailsVariation = json_decode($response);
    }
    $productDiscount = $productDetailsVariation[0]->wholesale_data->wholesale_price->wholesale_customer;
...
<?php if ($price_html = $product->get_price_html()) : ?>
    ...
    if (!in_array('wholesale_customer', (array) $user->roles)) : ?>
        ...
        <div class="price__container">
            ...
            <?php if ($productDetails->variations != NULL) : ?>
                <span class="price"><?= "$" . ag_getProductVariations($siteURL, $product); ?></span>

            <?php elseif ($productDetails->wholesale_data->wholesale_price->wholesale_customer != null) : ?>
